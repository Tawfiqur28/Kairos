...
import type { ActionPlan, Ikigai, EducationLevel } from '@/lib/types';
import { useLanguage } from '@/context/language-context';
import { Calendar, GraduationCap, Target, Clock, Award, TrendingUp, AlertCircle, Download, Share2, Printer, RefreshCw, Home, Briefcase, Trophy } from 'lucide-react';
...
export default function PlanPage() {
  ...
  const careerInfo = useMemo(() => {
    if (!plan) return null;
    return careerData.careers.find(c => c.title === plan.careerTitle);
  }, [plan]);
  ...
  const educationLevelMap: Record<string, string> = {
    ...
    notSpecified: t('ikigai.educationLevelNotSpecified')
  };
  ...
  const displayEducationLevel = plan?.educationLevel && educationLevelMap[plan.educationLevel] 
    ...
    : plan?.educationLevel || t('ikigai.educationLevelNotSpecified');
              ...
              <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                ...
                <div className="space-y-2 text-right">
                  <div className="text-2xl font-bold text-primary">
                    {t('plan.percentComplete', { percent: Math.round(overallProgress) })}
                  </div>
            ...
            <Card className="mb-6 border-primary/20">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <CardTitle>{t('nav.careers')}</CardTitle>
                    <CardDescription>
                      {t('careers.dialogDescription', { career: plan?.careerTitle })}
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    ...
                    <div className="font-semibold text-lg">{careerInfo.salaryRange || 'N/A'}</div>
                  </div>
                  <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    <div className="text-xs text-muted-foreground mb-1">Growth Outlook</div>
                    <div className="font-semibold text-lg">{careerInfo.growthOutlook || 'N/A'}</div>
                  </div>
                  <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    <div className="text-xs text-muted-foreground mb-1">Education Required</div>
                    <div className="font-semibold text-lg">{careerInfo.educationRequired || 'N/A'}</div>
                  </div>
                  <div className="p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                    <div className="text-xs text-muted-foreground mb-1">Market Demand</div>
                    <div className="font-semibold text-lg">
                      {careerInfo.marketDemand ? `${careerInfo.marketDemand}/10` : 'N/A'}
                    </div>
                  </div>
                </div>
                
                <div className="mt-4 pt-4 border-t">
                  <h4 className="text-sm font-medium mb-2">{t('careers.keySkills')}</h4>
                  <div className="flex flex-wrap gap-2">
                    {careerInfo.requiredSkills?.map((skill, index) => (
                      <Badge key={index} variant="secondary">
                        <TechnicalText text={skill} />
                      </Badge>
                    ))}
                  </div>
                </div>
        ...
        <motion.div variants={itemVariants}>
          <Tabs defaultValue="action-plan" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="action-plan">{t('nav.actionPlan')}</TabsTrigger>
              <TabsTrigger value="opportunities">{t('opportunities.tabTitle')}</TabsTrigger>
            </TabsList>
              ...
              <div className="space-y-6 mt-6">
                ...
                >
                  {plan.phases.map((phase, phaseIndex) => (
                    <motion.div key={phaseIndex} variants={itemVariants}>
                      <Card className={phaseProgress[phaseIndex] === 100 ? 'border-green-200 dark:border-green-800' : ''}>
                        <CardHeader>
                          <div className="flex justify-between items-start mb-2">
                            <CardTitle className="flex items-center gap-2">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {task.completed && (
                                  ...
                                  >
                                    ...
                                    <span className="text-xs text-green-600">{t('dashboard.ready')}</span>
                                  </motion.div>
                                )}
                                </AnimatePresence>
                              </div>
            ...
            <TabsContent value="opportunities">
              <OpportunitiesDashboard 
                ...
                educationLevel={plan.educationLevel as EducationLevel} 
              />
            </TabsContent>
          </Tabs>
        </motion.div>
